...
```{r}
unique(linkdata$hhincome)
```


```{r}
linkdata_new <- linkdata %>% 
  mutate(hhincome = recode(hhincome, 'Under 12000' = "Under $12,000", '12000-15999' = "$12,000-$15,999",'16,000-$19,999' = "$16,000-$19,999")) 
```

```{r}
unique(linkdata_new$hhincome)
```
...
```{r}
unique(linkdata$hhveh_clean)
```

%Delete #NULL

```{r}
unique(linkdata$oneway_all_clean)
```
...
linkdata_clean <- linkdata_new %>%
  filter(faretype != "#NULL!") %>%
  filter(purpose != "#NULL!") %>% 
  filter(hhincome != "#NULL!") %>%
  filter(hhvec_clean != "#NULL") %>%
  filter(oneway_all_clean != "#NULL")
...
library(dplyr)
mydata1 <- rename(linkdata_clean,'number_of_trips'='oneway_all_clean', 'vehicle'='hhveh_clean', "income"="hhincome")
