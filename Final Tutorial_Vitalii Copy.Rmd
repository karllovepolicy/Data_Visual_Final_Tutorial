...
<p style="text-align: center; font-size: 130%;"><b>
...
Then, we need to call the data we would use for the analysis. In our case, we import data from the csv file
...
str(linkdata) #checking the variables, their names and types
```
...
```{r}
absol_ft=table(mydata2$faretype, exclude = 'nothing') #absolute values
ft_freq=prop.table(absol_ft)*100 #proportion
ft_freq_df=as.data.frame(ft_freq) #changing to a data frame format
names(ft_freq_df)=c("faretype","pct") #changing names of data frame columns
ft_freq_df_ord=ft_freq_df[order(-ft_freq_df$pct), ] #ordering
ft_freq_df_ord
```
...
ft_plot4 = ft_plot3 + scale_y_continuous(limits = c(0,60),
                                         breaks = seq(0, 60, by = 10),
                                         labels = scales::unit_format(suffix = '%')) +
                      scale_x_discrete(limits = ft_freq_df_ord$faretype)
ft_plot5 = ft_plot4 + geom_text(aes(y = pct,
                                label = paste0(round(pct,2), '%')), 
                                hjust= -0.15, 
                                size = 2.75)
ft_plot6 = ft_plot5 + theme(panel.background = element_rect(fill = "white", color = "black"),
...
```{r barplotfarepurpose, fig.align='center'}
...
```{r compare different models, fig.align='center'}
...
Here should be 1 paragraph of the conclusion
